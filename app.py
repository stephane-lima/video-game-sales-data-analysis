...
def clean_data(df):
    # This line removes rows that contain missing values, but the result is not assigned back to the DataFrame.
    df.dropna()

    # These lines were kept as examples for filling missing values, but they are currently turned off.
...
def question_five(df):
    ...
    # Build a table that stores the total sales for each region.
    ...
    with col_graph1:
        # Create a horizontal bar chart for regional sales.
        fig = px.bar(
            regional_sales,
